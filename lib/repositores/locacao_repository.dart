...
class LocacaoRepository {
  Future<bool> gravar(LocacaoModel locacaoModel) async {
    ...
    try {
      final response = await http.post(Uri.parse(uri), headers: header, body: body);
  ...
  Future<List<LocacaoModel>> buscarlocacoes() async {
    ...
    var uri = 'http://erig.de.br/api/v1/locacoes/$userId';
    var header = {
      'Content-type': 'application/json',
      'Authorization': 'Bearer $token'
    };
    ...
    try {
      ...
      final List<LocacaoModel> locacoes = locacoesList.map<LocacaoModel>((locacaoMap) {
        return LocacaoModel.fromJson(locacaoMap);
      }).toList();
